...
class KhopkhuaColors {
  ...
  static const navyMid  = Color(0xFF1E3A6E);   // ສວ່າງຂຶ້ນ
  static const navyLight= Color(0xFF2A4A8A);   // ສວ່າງຂຶ້ນ
  static const emerald  = Color(0xFF0F6E56);
  static const emeraldL = Color(0xFF1D9E75);
  static const gold     = Color(0xFFD4AF37);
  static const goldL    = Color(0xFFF0C040);
  static const goldD    = Color(0xFFA88820);
  static const cream    = Color(0xFFFFF8E7);
  static const white    = Color(0xFFFFFFFF);
  static const cardBg   = Color(0xFF1A3360);   // card ສວ່າງຂຶ້ນ
  static const pageBg   = Color(0xFF0F2347);   // page bg ສວ່າງຂຶ້ນ
}
...
class _KhopkhuaScreenState extends State<KhopkhuaScreen>
  ...
  final List<TeamMember> _team = [
    TeamMember(name: 'ນາງ ສຸກັນຍາ', role: 'ຜູ້ຈັດການ', phone: '+856 20 777 77421', color: Color(0xFFD4AF37), initials: 'ສ'),
    TeamMember(name: 'ທ້າວ ບຸນມີ', role: 'ນາຍໜ້າ Senior', phone: '+856 20 555 11111', color: Color(0xFF0F6E56), initials: 'ບ'),
    TeamMember(name: 'ນາງ ຈັນທະລາ', role: 'ນາຍໜ້າ', phone: '+856 20 555 22222', color: Color(0xFF1D9E75), initials: 'ຈ'),
    TeamMember(name: 'ທ້າວ ວິໄລ', role: 'ນາຍໜ້າ', phone: '+856 20 555 33333', color: Color(0xFF2A4A8A), initials: 'ວ'),
    TeamMember(name: 'ນາງ ດາວ', role: 'ການຕະຫຼາດ', phone: '+856 20 555 44444', color: Color(0xFFD85A30), initials: 'ດ'),
    TeamMember(name: 'ທ້າວ ສົມພອນ', role: 'ການຕະຫຼາດ', phone: '+856 20 555 55555', color: Color(0xFF7F77DD), initials: 'ສ'),
    TeamMember(name: 'ນາງ ມະນີ', role: 'ບັນຊີ', phone: '+856 20 555 66666', color: Color(0xFFBA7517), initials: 'ມ'),
    TeamMember(name: 'ທ້າວ ຄຳ', role: 'ຊ່ວຍການ', phone: '+856 20 555 77777', color: Color(0xFF993556), initials: 'ຄ'),
  ];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: KhopkhuaColors.pageBg,
      body: Column(children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabCtrl,
            children: [_buildListingsTab(), _buildAddTab(), _buildTeamTab(), _buildStatsTab()],
          ),
        ),
      ]),
    );
  ...
  Widget _buildTabs() {
    return Container(
      ...
      child: TabBar(
        ...
        unselectedLabelColor: KhopkhuaColors.white.withOpacity(0.5),
        indicatorColor: KhopkhuaColors.gold,
        indicatorWeight: 2.5,
        labelStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.w600),
        tabs: [
          Tab(text: 'ຊັບ (${_listings.length})'),
          const Tab(text: '+ ເພີ່ມ'),
          const Tab(text: 'ທີມງານ'),
  ...
  Widget _buildListingCard(PropertyListing p) {
    return Container(
      ...
      decoration: BoxDecoration(
        color: KhopkhuaColors.cardBg,
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: KhopkhuaColors.gold.withOpacity(0.4), width: 0.8),
      ),
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        // Card header
        Container(
      ...
      children: [
        ...
        Container(
          ...
          decoration: BoxDecoration(
            color: KhopkhuaColors.cardBg,
            borderRadius: BorderRadius.circular(16),
            border: Border.all(color: KhopkhuaColors.gold.withOpacity(0.5), width: 1),
          ),
          child: Row(children: [
            Container(
              width: 50, height: 50,
              ...
              child: ClipRRect(
                borderRadius: BorderRadius.circular(8),
                child: Image.asset('assets/images/khopkhua_logo.png', fit: BoxFit.cover,
                  errorBuilder: (_, __, ___) => const Icon(Icons.home, color: KhopkhuaColors.gold, size: 28)),
              ),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('ທີມງານ Khopkhua', style: TextStyle(
                color: KhopkhuaColors.gold, fontSize: 15, fontWeight: FontWeight.w600)),
              Text('ທີມງານທັງໝົດ ${_team.length} ຄົນ · ພ້ອມໃຫ້ບໍລິການ 24/7',
                style: TextStyle(color: KhopkhuaColors.white.withOpacity(0.6), fontSize: 11)),
            ])),
            Container(
              padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
              decoration: BoxDecoration(
                color: KhopkhuaColors.emerald.withOpacity(0.3),
                borderRadius: BorderRadius.circular(20),
                border: Border.all(color: KhopkhuaColors.emeraldL, width: 0.5),
              ),
              child: Text('${_team.length} ຄົນ', style: const TextStyle(
                color: KhopkhuaColors.emeraldL, fontSize: 12, fontWeight: FontWeight.w600)),
            ),
          ]),
        ),
        // Team grid
        GridView.builder(
          ...
          itemBuilder: (ctx, i) => _buildTeamCard(_team[i]),
        ),
      ],
    );
  }

  Widget _buildTeamCard(TeamMember m) {
    return Container(
      decoration: BoxDecoration(
        color: KhopkhuaColors.cardBg,
        borderRadius: BorderRadius.circular(14),
        border: Border.all(color: m.color.withOpacity(0.5), width: 0.8),
      ),
      padding: const EdgeInsets.all(12),
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        Container(
          ...
          child: Center(child: Text(m.initials, style: TextStyle(
            color: m.color, fontSize: 18, fontWeight: FontWeight.w700))),
        ),
        const SizedBox(height: 8),
        Text(m.name, style: const TextStyle(
          color: KhopkhuaColors.cream, fontSize: 12, fontWeight: FontWeight.w500),
        ...
        Container(
          ...
          child: Text(m.role, style: TextStyle(color: m.color, fontSize: 9, fontWeight: FontWeight.w500)),
        ),
        const SizedBox(height: 6),
        // Call button
        GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: KhopkhuaColors.emerald.withOpacity(0.2),
              borderRadius: BorderRadius.circular(8),
              border: Border.all(color: KhopkhuaColors.emeraldL.withOpacity(0.5), width: 0.5),
            ),
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.phone, color: KhopkhuaColors.emeraldL, size: 10),
              const SizedBox(width: 3),
              Text('ໂທ', style: const TextStyle(color: KhopkhuaColors.emeraldL, fontSize: 10)),
            ]),
          ),
        ),
      ]),
  ...
  Widget _statGrid() {
    final stats = [
      ...
      ('ທີມງານ', '${_team.length}', KhopkhuaColors.emerald),
    ];
    return GridView.count(
      ...
      children: stats.map((s) => Container(
        decoration: BoxDecoration(
          ...
          border: Border.all(color: s.$3.withOpacity(0.4), width: 0.8),
        ),
        padding: const EdgeInsets.all(14),
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
          Text(s.$2, style: TextStyle(color: s.$3, fontSize: 22, fontWeight: FontWeight.w700)),
          Text(s.$1, style: TextStyle(color: KhopkhuaColors.white.withOpacity(0.6), fontSize: 11)),
        ]),
      )).toList(),
    );
  }
...
// ==========================================
class TeamMember {
  final String name, role, phone, initials;
  final Color color;
  const TeamMember({
